...
export default class IncidentsDailyOverview extends React.Component {
  renderIncident(incident) {
    return (
      <div className={`incidents-daily__incident incidents-daily__incident--impact-${incident.impact}`} key={incident.slug}>
        ...
        </h6>
        {
          incident.updates.map(update => (
            <div className="incidents-daily__incident__update" key={update.content}>
              ...
              <p className="incidents-daily__incident__update__timestamp">
                {format(update.date, 'MMM D, hh:mm Z')}
              </p>
            </div>
          ))
        }
  ...
  renderDay(day) {
    ...
    return (
      <div className="incidents-daily__day" key={day}>
        <h5 className="incidents-daily__day__title">{format(day, 'MMMM D, YYYY')}</h5>
        <div>
          {
            dayIncidents.length === 0 ?
              <p className="incidents-daily__day__no-incidents">No incidents reported.</p> :
  ...
  render() {
    ...
    return (
      <div className="incidents-daily">
        <div className="incidents-daily__title">
          Past incidents
        </div>
